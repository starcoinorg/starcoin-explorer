...
export default function reducers(state: any = initState, action: any) {
  switch (action.type) {
    ...
    case types.SET_ADDRESS_TRANSACTION_LIST: {
      return { ...state, addressTransactionList: action.payload };
    }
    ...
    case types.GET_ADDRESS_TRANSACTION_LIST_REQUEST: {
      return { ...state, isLoadingMore: true };
    }
    ...
    case types.GET_ADDRESS_TRANSACTION_LIST_SUCCESS:
    case types.GET_ADDRESS_TRANSACTION_LIST_FAILURE: {
